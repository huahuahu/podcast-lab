...
| 环节 | 选用 | 为什么 |
|---|---|---|
| **STT + Diarize** | Azure `gpt-4o-transcribe-diarize`（SSE 流式） | 一个 API 同时返回文字 + 时间戳 + 说话人；免本地 pyannote、免 HF token、免长时间跑模型。也可改用 **OpenAI 直连**（`api.openai.com` 的 `gpt-4o-transcribe`），只需换 endpoint + `OPENAI_API_KEY`；但国内走 VPN 直连经常断流，长音频建议优先 Azure。 |
