...
| 环节 | 选用 | 为什么 |
|---|---|---|
| **STT + Diarize** | Azure `gpt-4o-transcribe-diarize`（SSE 流式） | 一个 API 同时返回文字 + 时间戳 + 说话人。**Azure 不能走代理**（SSE 会被别、长音频会被拒）；脱本 yt-dlp 又必须走代理。现在脚本在包里自己 unset 代理，外部保持 `HTTPS_PROXY=...` 即可。 |
