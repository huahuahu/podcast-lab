...
| `reassign_speakers_llm.py` | LLM 跨 chunk 校正 speaker 标签（2 人节目用，host/guest） |
| `align_speakers_multi.py` | 多人节目（>2 speakers）跨 chunk speaker 对齐，靠 `series.json` 里的 `personas` 画像 |
...
| 环节 | 选用 | 为什么 |
|---|---|---|
| **STT + Diarize** | Azure `gpt-4o-transcribe-diarize`（SSE 流式） | 一个 API 同时返回文字 + 时间戳 + 说话人。**Azure 不能走代理**（SSE 会被别、长音频会被拒），但脚本在包里自己 unset 代理，外部什么都不需要动。 |
| **翻译** | GitHub Copilot `gpt-5.4` | 公司订阅免费；口语化、保留术语、上下文连贯 |
| **TTS** | `edge-tts` + `zh-CN-YunyangNeural` / `zh-CN-XiaoxiaoNeural` | 免费白嫖，多音色，音质自然 |

> ⚠️ Azure diarize 每个 chunk 的 `A/B/C` 标签独立，跨 chunk 不保证同一个人是同一个字母。2 人节目走 `reassign_speakers_llm.py`；多人节目（如 All-In）走 `align_speakers_multi.py` + `series.json` 里的 `personas` 映射为人名。
...
- **RSS 引用的最终位置**：`docs/assets/covers/<slug>.png`（进 git，GitHub Pages 提供）
- **中间产物**：`projects/<slug>/cover.png`（gitignored，本地重跑能再生）
- 尺寸：1400×1400（iTunes 最低 1400×1400，最大 3000×3000）
- 来源：`enrich/cover_fetch.sh` 自动处理（原图 thumbnail → og:image → AI 兜底）；手工复用优先 YouTube `https://i.ytimg.com/vi/<VIDEO_ID>/maxresdefault.jpg`
- 发布前别忘 `cp projects/<slug>/cover.png docs/assets/covers/<slug>.png` 然后 git add
...
export CHUNK_SEC=1200  # Azure 切片长度，默认 300s；实际推荐 1200（20 分钟）；0 = 整台不切（临界不稳）
```
...
默认从 `configs/series.json` 里按系列 regex 匹配读取（默认兑底在 `scripts/prepare_multivoice.py`）。加新音色前必须用 `python3 -m edge_tts --list-voices` 核对名字是否存在（`zh-CN-YunhaoNeural` 这种不存在的名字会让 TTS 直接崩）。

常用中文音色：

| Voice | 描述 | 典型用途 |
|---|---|---|
| `zh-CN-YunyangNeural` | 云扬，男，新闻播报风 | 主持人 / Chamath |
| `zh-CN-YunjianNeural` | 云剑，男，运动激情 | Jason |
| `zh-CN-YunxiNeural`   | 云希，男，年轻阳光 | Sacks / narrator |
| `zh-CN-XiaoxiaoNeural`| 晓晓，女声 | host / Unknown fallback |
| `zh-CN-XiaoyiNeural`  | 晓伊，女声 | 其他女角色 |
| `zh-TW-YunJheNeural`  | 台湾男声 | Freeberg（满普通话男声之后的第 5 个选项） |
| `zh-HK-WanLungNeural` | 香港男声 | 备选 |
...
截至 EP19（All-In · OpenAI Misses），全部集数请订阅 RSS：
https://huahuahu.github.io/podcast-lab/rss.xml
或浏览发布页：
https://github.com/huahuahu/podcast-lab/releases

## 集数编号约定（EPnn）

- 所有 RSS / GitHub Release / 对外标题都以 `EPnn · 中文标题` 开头，两位数补零
- EP01–EP19 已按 `pubDate` 升序回填
- 发新一集前先查最大号：
  ```bash
